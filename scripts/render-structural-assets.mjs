 ...
 *
 * Los 96 PNG del repositorio venían horneados con la paleta de StructureCo
 ...
 * `window.__STRUCTURECO_RENDER_ASSET__`. Este script lo conduce con un
...
await page.waitForFunction(() => typeof window.__STRUCTURECO_RENDER_ASSET__ === 'function', null, { timeout: 60_000 });

const ids = await page.evaluate(() => [...(window.__STRUCTURECO_THREE_ASSET_IDS__ ?? [])]);
...
for (const theme of ['day', 'night']) {
  for (const id of ids) {
    const dataUrl = await page.evaluate(
      ([assetId, renderTheme]) => window.__STRUCTURECO_RENDER_ASSET__(assetId, renderTheme),
      [id, theme],
    );
